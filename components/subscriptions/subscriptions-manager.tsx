...
export function SubscriptionsManager() {
  ...
  return (
    <div className="relative flex flex-col gap-4 p-4 sm:p-6 lg:p-8">
      {/* ─── Top Header Bar: Status Tabs & Add Button (Matching Expense & Portfolio) ─── */}
      <div className="flex min-w-0 flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between">
        ...
        {/* Right: Prominent + Add Subscription Button right at the top */}
        <div className="flex shrink-0 items-center gap-2">
          <Button
            size="sm"
            onClick={handleOpenAdd}
            className="h-9 w-full gap-1.5 rounded-xl bg-indigo-600 px-3.5 text-sm font-bold text-white shadow-sm shadow-indigo-600/25 transition-all hover:bg-indigo-500 active:scale-[0.98] sm:w-auto"
          >
      ...
      <div className="grid grid-cols-2 gap-2 sm:gap-3 lg:grid-cols-4">
        ...
        {/* Yearly Projection */}
        <div className="flex flex-col justify-between rounded-2xl border border-sky-200/60 bg-gradient-to-br from-sky-50 to-sky-100/40 p-3 sm:p-3.5 dark:border-sky-800/40 dark:from-sky-950/40 dark:to-sky-900/20">
          <div>
            <div className="mb-1 flex items-center gap-1.5 sm:gap-2">
              <div className="flex size-6 sm:size-7 shrink-0 items-center justify-center rounded-lg bg-sky-600/10 text-sky-700 dark:text-sky-400">
                <TrendingUp className="size-3.5 sm:size-4" />
              </div>
              <span className="truncate text-[10px] font-bold uppercase tracking-wider text-sky-600/80 dark:text-sky-400/80">
                Yearly Projection
              </span>
            </div>
            <p className="font-mono text-base font-black tracking-tight text-sky-900 dark:text-sky-300 sm:text-xl">
        ...
        <div className="flex flex-col justify-between rounded-2xl border border-emerald-200/60 bg-gradient-to-br from-emerald-50 to-emerald-100/40 p-3 sm:p-3.5 dark:border-emerald-800/40 dark:from-emerald-950/40 dark:to-emerald-900/20">
          ...
          <p className="mt-0.5 line-clamp-1 text-[10px] text-slate-500 dark:text-slate-400 sm:text-[11px]">
            {subscriptions.length - activeSubs.length} paused / cancelled
          </p>
        </div>
        ...
        <div className="flex flex-col justify-between rounded-2xl border border-amber-200/60 bg-gradient-to-br from-amber-50 to-amber-100/40 p-3 sm:p-3.5 dark:border-amber-800/40 dark:from-amber-950/40 dark:to-amber-900/20">
          <div>
            ...
            <p className="font-mono text-base font-black tracking-tight text-amber-900 dark:text-amber-300 sm:text-xl">
              ...
              <span className="text-xs font-normal text-amber-600/70 dark:text-amber-400/70"> dues</span>
            </p>
          </div>
          <p className="mt-0.5 line-clamp-1 text-[10px] text-slate-500 dark:text-slate-400 sm:text-[11px]">
      ...
      <div className="flex flex-col gap-2.5">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="relative min-w-0 flex-1">
            ...
            <Input
              ...
              className="h-9 rounded-xl border-slate-200 bg-white pl-9 pr-8 text-xs shadow-2xs placeholder:text-slate-400 focus-visible:border-indigo-500 dark:border-slate-800 dark:bg-slate-900"
            />
            {searchQuery && (
              <button
          ...
          {/* Right Toolbar: View Switcher + Select All in Single Row */}
            ...
            {filteredSubscriptions.length > 0 && (
              <label className="flex cursor-pointer items-center gap-1.5 rounded-xl border border-slate-200/80 bg-white px-2.5 py-1.5 text-xs font-bold text-slate-600 shadow-2xs transition-colors hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800">
                <Checkbox
                  ...
                  className="size-3.5 rounded border-slate-300 data-[state=checked]:bg-indigo-600 data-[state=checked]:border-indigo-600"
                />
        ...
        {/* Category Filter Pills (Guaranteed Single-Line Horizontal Scroll) */}
        <div className="flex w-full min-w-0 flex-nowrap items-center gap-1.5 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {["All", ...SUBSCRIPTION_CATEGORIES].map((cat) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "inline-flex shrink-0 whitespace-nowrap items-center gap-1.5 rounded-xl border px-3 py-1.5 text-xs font-bold transition-all",
                  active
